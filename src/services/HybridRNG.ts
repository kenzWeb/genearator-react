import type {EntropyData, EntropySource} from '../types'
...
export class HybridRNG {
	...
	async collectEntropy(
		callback?: (source: EntropySource) => void,
	): Promise<EntropyData> {
	...
	generateRandomNumbers(count: number): number[] {
		...
		for (let i = 0; i < count; i++) {
			...
			const entropy2 =
				this.entropyPool[(poolIndex + 1) % this.entropyPool.length]
			const entropy3 =
				this.entropyPool[(poolIndex + 2) % this.entropyPool.length]
